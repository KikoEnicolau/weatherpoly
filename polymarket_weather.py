...
def get_polymarket(city_poly, date):
    """
    Fetch Polymarket event and consolidate individual Yes/No markets into
    a single list of {temp, yes_price, no_price, volume, question}.
    Each sub-market is like "Will highest temp be 13°C?" with outcomes ["Yes","No"].
    groupItemTitle gives us the clean label e.g. "13°C".
    """
    ...
    try:
        ...
        for m in markets:
            ...
            yes_price = prices[0]
            no_price  = prices[1]
            label     = m.get("groupItemTitle", "")   # e.g. "13°C", "8°C or below"
...
def ai_analysis(city, today_max, today_min, today_precip,
    ...
    for label, date, max_t, precip, rows, slug in [
        ...
        scored = []
        for r in rows:
            dist = r["mkt_temp"] - frounded
            if dist == 0:
                action = "SIM"; buy_c = r["yes_c"]; edge = 3
            elif dist == -1 and precip >= 50:
                action = "SIM"; buy_c = r["yes_c"]; edge = 2
            elif dist == 1 and precip < 30:
                action = "SIM"; buy_c = r["yes_c"]; edge = 1
            else:
                action = "NÃO"; buy_c = r["no_c"]
                edge = max(0, 3 - abs(dist)) if r["yes_price"] >= 0.10 else 0
            payout = 100 - buy_c
            value  = edge * payout / 100.0
            scored.append({**r, "action": action, "buy_c": buy_c, "payout": payout,
                           "dist": dist, "value": value})

        top2       = sorted(scored, key=lambda x: -x["value"])[:2]
        ...
        def explain(p):
            ...
            if p["action"] == "SIM":
                if p["dist"] == 0:
                    why = f"Previsão bate exato em {frounded}°C — mercado a {p['yes_pct']:.0f}%, bom valor em SIM."
                elif p["dist"] == -1:
                    why = f"Com {precip:.0f}% de chuva, máxima deve ficar em {p['mkt_temp']}°C — SIM alinhado."
                else:
                    why = f"Forecast favorece {p['mkt_temp']}°C — SIM por {p['buy_c']}¢ paga {p['payout']}¢."
            else:
                why = (
                    f"Forecast aponta {eff_max:.1f}°C, mas mercado dá {p['yes_pct']:.0f}% "
                    f"para {p['mkt_temp']}°C ({dist_lbl}). "
                    f"NÃO por {p['buy_c']}¢ paga {p['payout']}¢ se o forecast estiver certo."
                )
            return (
