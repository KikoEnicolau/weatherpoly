...
def get_polymarket_odds(city_name, target_date):
    """Search Polymarket Gamma API for temperature markets for a city."""
    date_str = target_date.strftime("%B %-d").lower()  # e.g. "march 15"
    search_query = f"highest temperature in {city_name.lower()} on {date_str}"
    ...
    try:
        ...
        for m in markets:
            ...
            if city_name.lower().split()[0] in q and ("temperature" in q or "highest" in q):
...
for i, city in enumerate(selected):
    ...
    with cols[i % 2]:
        ...
        poly    = get_polymarket_odds(city, target_date)
        ...
        if weather and "daily" in weather:
            ...
            today_idx = dates.index(target_str) if target_str in dates else 1
